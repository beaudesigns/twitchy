...
describe('TwitchTV Library', function () {
	...
	describe('REST API', function () {
		describe("Block/Ignore control", function () {
			...
			it('userFollowsChannel', function (done) {
				new Twitchy(credentials).auth().userFollowsChannel("telamohn", done);
			});
		});
		...
		describe("Streams control", function () {
			it('getStream', function (done) {
				new Twitchy(credentials).auth().getStream("numrii", done);
			});
		});
